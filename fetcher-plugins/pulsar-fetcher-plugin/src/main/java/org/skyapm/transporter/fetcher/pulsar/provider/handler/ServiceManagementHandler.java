 ...
 */
@Slf4j
public class ServiceManagementHandler extends AbstractPulsarHandler {
    ...
    public void handle(Message<byte[]> message) {
        try {
            if (message.getKey().startsWith("register-")) {
                serviceReportProperties(InstanceProperties.parseFrom(message.getData()));
            } else {
                keepAlive(InstancePingPkg.parseFrom(message.getData()));
            }
        } catch (Exception e) {
